...
func (st *SSHTools) GetTools() ([]runtimeport.Tool, error) {
	...
	// 执行远程命令工具
	executeTool, err := runtimeport.InferTool("ssh_execute", "在远程服务器执行 shell 命令。命令执行前会进行安全检查，拒绝执行危险命令。支持设置超时时间（默认60秒，最大300秒）", st.SSHExecute)
	if err != nil {
		return nil, err
	}
