...
func TestMarkdownCollectorCollectsMessagesToolsAndErrors(t *testing.T) {
	...
	eventsToSend := []Event{
		{Type: EventAssistantText, AgentName: "assistant", Content: "hello"},
		{Type: EventAssistantText, AgentName: "assistant", Content: " world"},
		{
		...
		{Type: EventToolCallCompleted, AgentName: "assistant", ToolCallID: "tool-1", Content: `{"message":"搜索完成","results":[{"title":"Go","url":"https://go.dev","summary":"语言"}]}`},
		{Type: EventSystemNotice, AgentName: "assistant", Content: "coder", Notice: &domainevent.NoticePayload{Code: "transfer"}},
...
func TestMarkdownCollectorIgnoresNonOutputDeltaAndInternalTool(t *testing.T) {
	callback, result := NewMarkdownCollector()
	if err := callback(Event{Type: EventAssistantReason, DeltaKind: domainevent.DeltaReasoning, AgentName: "assistant", Content: "hidden"}); err != nil {
		t.Fatal(err)
	}
