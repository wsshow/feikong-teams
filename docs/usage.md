...
- 完整的 Markdown 渲染支持
- 一键导出对话历史为 HTML 文件
- 任务运行中可继续发送任务；输入默认作为续问排队，有未执行内容时显示队列面板，支持转为转向、编辑、删除和调整顺序
- 响应式设计，支持移动端访问
- 可折叠侧边栏和模式切换
- 可选的登录认证（通过环境变量配置）
...
任务运行中直接输入内容并按 Enter 会加入转向队列，并立即显示在终端消息流里；下一次模型调用前会合并消费当前所有未执行转向。运行中按 `Esc` 会暂停当前任务，并将尚未执行的转向消息回填到输入框，便于继续修改后重新提交。
