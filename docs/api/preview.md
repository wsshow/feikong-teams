...
直接渲染预览文件内容，适用于 HTML 文件的完整预览（支持相对路径资源加载）。当 filepath 为空或 `/` 时返回主文件；否则解析为主文件目录下的相对路径资源。
...
文件内容以 `Content-Disposition: inline` 方式返回（浏览器直接渲染）。首次密码认证通过后会设置 Cookie（有效期 1 小时），使 iframe 内相对路径资源自动携带凭据。
