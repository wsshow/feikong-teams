...
- **团队模式** (`team`)：使用内置智能体（`coder`、`shell`、`writer`、`summarizer` 始终可用；`researcher`、`analyst`、`remote`、`generalist` 需在配置文件中启用）。自定义智能体同样加入注册表，可通过 `@` 或 `agent` 子命令单独使用
- **深度模式** (`deep`)：使用深度探索协调者，能深入分析问题并协调多个智能体解决复杂任务。自定义智能体同样可通过 `@` 单独使用
- **讨论模式** (`group`)：使用 `[[roundtable.members]]` 配置的圆桌会议成员。自定义智能体可通过 `@` 单独使用
- **自定义模式** (`custom`)：使用 `[custom.moderator]` 主持人，将 `[[custom.agents]]` 自定义智能体封装为 AgentTool 按需调用
