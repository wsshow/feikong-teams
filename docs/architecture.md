...
工具组目录信息和工厂必须通过同一个注册项进入 `internal/app/tools.ToolGroupRegistry`，避免目录、策略和运行时解析出现双来源。需要具体技术实现的工具组（如 `schedule_*`）由 `internal/bootstrap/tools` 连接 adapter 与 app 注册表，`internal/app/tools` 禁止 import `internal/adapters/*`。MCP 动态工具通过 `internal/ports/tools.MCPProvider` 注入，具体 MCP client、协议初始化和工具桥接归属 `internal/adapters/tools/mcp`。
