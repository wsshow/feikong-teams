...
- `tools/excel`、`tools/doc`、`tools/fetch`、`tools/search`：文件格式引擎、HTTP 抓取和搜索实现位于 `internal/adapters/tools/builtin/*`，由 `internal/bootstrap/tools` 注册；`internal/app/tools` 只保留注册表、目录、策略和纯用例工具。
...
`internal/runtime/checkpoint` 只提供内存存储、命名空间等 runtime 无关实现。文件系统只是 adapter；核心不直接拼路径，也不依赖具体文件历史实现。
...
- 对工具迁移保持克制，只迁移真正带外部协议、IO、生命周期或替换价值的实现。
