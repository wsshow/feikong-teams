...
根 `events` 包只保留外层入口兼容的导出门面和事件类型别名；它不是内核实现位置，不允许被 `internal/**` 导入。根 `events/view` 属于 CLI 展示层，也不能被用例层或 runtime 层导入。
...
- `events/view`：CLI/Markdown 展示层已直接使用 `internal/domain/event` 与 `internal/domain/message`，禁止再通过 `agentcore` 旧门面获取 ToolCall；后台调度结果收集已迁入 `internal/app/schedule`，内部用例层不再复用根展示包。
