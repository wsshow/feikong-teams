...
根 `events` 包只保留外层入口兼容的导出门面和事件类型别名；它不是内核实现位置，不允许被 `internal/**` 导入。CLI 事件展示位于 `internal/adapters/transport/cli/eventview`，不能放回根 `events/view`。
...
- `commands/*`：CLI 命令层只保留参数解析和应用生命周期连接，直接调用 `internal/adapters/transport/cli/runtime`、`internal/ports/runtime` 与 `internal/domain/message`，禁止再通过 `agentcore` 或根 `cli` 旧门面获取 Runner/Message/ToolCall/InterruptMetadata。
...
- `server`、`commands`、`channels` 不得 import Eino runtime adapter；CLI 交互运行时只能位于 `internal/adapters/transport/cli/runtime`。
