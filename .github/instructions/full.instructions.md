...
2. **新增环境变量更新 .env.example**: 添加新环境变量时，必须在 `common/common.go` 的 `GenerateExampleEnv` 中添加对应条目和注释
...
FEIKONG_CODER_ENABLED         # 启用代码专家
FEIKONG_CMDER_ENABLED         # 启用命令行专家
FEIKONG_ANALYST_ENABLED       # 启用数据分析师
FEIKONG_ASSISTANT_ENABLED     # 启用个人全能助手
FEIKONG_SSH_VISITOR_ENABLED   # 启用 SSH 专家
FEIKONG_SSH_HOST              # SSH 主机
FEIKONG_SSH_USERNAME          # SSH 用户名
FEIKONG_SSH_PASSWORD          # SSH 密码
FEIKONG_MEMORY_ENABLED        # 启用长期记忆
FEIKONG_LOGIN_ENABLED         # 启用 Web 登录认证
FEIKONG_LOGIN_SECRET          # JWT 密钥
...
- [ ] 无未使用的 import/变量
- [ ] 功能变更已更新 README.md
- [ ] 新环境变量已添加到 `common/common.go` 的 `GenerateExampleEnv`
