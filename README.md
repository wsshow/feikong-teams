...
![fkteams Web 主界面](./docs/images/fkteams_web_main.png)

![fkteams Web 任务执行](./docs/images/fkteams_web_task.png)

### 终端 TUI

![fkteams 终端 TUI](./docs/images/fkteams_tui_main.png)
...
- **技能管理**：Web 技能页支持搜索安装市场技能，也可创建、编辑和删除本地自定义技能；配置默认对话模型后，可通过 AI 根据需求一键生成并创建自定义 Skill
- **文件分享**：Web 文件管理可创建带过期时间和访问密码的文件分享链接
- **自定义智能体配置**：Web 设置页按智能体、圆桌、深度、权限等独立页签组织配置，可手动或通过 AI 批量创建智能体草稿，并可在描述和系统提示词字段中使用 AI 按要求改写内容；同时支持通过搜索、多选和工具说明快速配置内置工具、MCP 工具以及按智能体隔离的 SSH 连接
...
- 文件、命令、Git、SSH 等高风险能力会经过工具安全策略和人工确认流程；Web 设置页可按类别配置自动允许审批，被拒绝的操作不会被自动重试。
