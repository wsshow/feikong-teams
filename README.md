...
| 智能体  | 说明                                         | 默认启用 |
| ------- | -------------------------------------------- | -------- |
| `@小码` | 资深软件工程师，擅长代码实现、调试和重构     | ✓        |
| `@小令` | 命令行专家，根据操作系统环境执行合适的命令   | ✓        |
| `@小说` | 故事创作专家，擅长构思和撰写引人入胜的故事   | ✓        |
| `@小简` | 总结专家，擅长提炼和总结复杂信息             | ✓        |
| `@小搜` | 搜索专家，通过 DuckDuckGo 检索信息并深度分析 | 配置启用 |
| `@小析` | 数据分析专家，Excel 和 Python 脚本数据处理   | 配置启用 |
| `@小访` | 远程访问专家，SSH 连接远程服务器和系统管理   | 配置启用 |
| `@小助` | 个人全能助手，支持子任务并行分发执行         | 配置启用 |
